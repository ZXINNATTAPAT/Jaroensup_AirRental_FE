...
const EditOrder = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        const [usersResponse, productsResponse, orderResponse] =
          await Promise.all([
    ...
    <div className="container mx-auto p-8">
      <div className="p-8 rounded-lg shadow-lg w-full mx-auto font-prompt bg-base-100 h-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-md">
          {items.map((item, index) => (
            <div key={index} className="flex items-center space-x-4">
              <div className="flex-1">
                <label className="label">
                  <span className="label-text">
                    {translations[language].productLabel}
                  </span>
                </label>
                <select
                ...
                >
                  ...
                  {getFilteredProducts(index).map((product, index) => (
                    <option key={index + 1} value={product.product_id}>
                      {product.product_id}.{" "}
                      {translations[language].selectProductOption(
                        product.name,
              ...
              <div className="flex-1">
                <label className="label">
                  <span className="label-text">
                    {translations[language].totalPriceLabel}
                  </span>
                </label>
                <input
                  type="number"
          ...
          <div className="mt-4">
            <strong>{translations[language].totalOrderPrice}: </strong>{" "}
            {totalOrderPrice}
          </div>
