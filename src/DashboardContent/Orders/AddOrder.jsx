...
const AddOrder = () => {
  ...
  const handleItemChange = async (index, field, value) => {
    ...
    setItems(newItems);
    setTotalOrderPrice(
      newItems.reduce((total, item) => total + item.total_price, 0)
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${apiUrl}/v2/orders`, {
        user_id: userId,
        items: items.map(
          ({ productId, quantity, price, total_price, name }) => ({
      ...
      if (response.status === 201) {
        const orderId = response.data.orderId; // ดึง order_id จาก response
        const action = `สร้างคำสั่งซื้อไอดี ${orderId}`;
        ...
        setUserId("");
        setItems([
          { productId: "", name: "", quantity: 0, price: 0, total_price: 0 },
        ]);
    ...
    <div className="container mx-auto p-8">
      ...
      <div className="flex  w-full my-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-md">
          ...
          {items.map((item, index) => (
            <div key={index} className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                <label className="label">
                  <span className="label-text">
                    {translation[language].totalPrice}
                  </span>
                </label>
                <input
                  type="number"
          ...
          <div className="mt-4">
            <strong>{translation[language].totalPrice}: </strong>{" "}
            {totalOrderPrice}
          </div>
